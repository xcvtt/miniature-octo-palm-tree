...
<details>
  <summary>Текст для LLM</summary>
	Debian 12  
	Дай мне пошаговую инструкцию по минимальной настройке VPS - создание пользователя, защиту SSH, конфигурацию фаерволла итд
</details>
...
> [!spoiler]- Если вы устанавливали 3X-UI до этого
> В старых версиях установочного скрипта 3X-UI было сразу несколько способов выстрелить себе в ногу. Панель шла с кредами admin/admin, по http, сервис подписок включен по умолачанию по известному пути /sub/. Есть неплохая вероятность, что ваша панель скомпроментирована и ваши конфиги лежат в открытом доступе [где-то здесь](https://github.com/zieng2/wl)
...
### Панель Remnawave
[Remnawave](https://github.com/remnawave/panel) - инструмент для оркестрации множества VPS с xray-core (Remnawave node). Remnawave позволяет поднимать сколько угодно серверов с Remnawave node и управлять всеми с единого сервера-панели.

У инструмента много различных фич и возможностей, дополнительно можно ознакомиться по [ссылке](https://docs.rw/docs/learn-en/quick-start). На мой взгляд больше подходит для коммерческого использования, 3x-ui покрывает 99.9% нужд обычного пользователя.
...
## Резюмируя
Проводные и мобильные ISP продолжают внедрять шейпинг трафика к большинству зарубежных IP с использованием грубой [эвристики](https://github.com/net4people/bbs/issues/490). Это почти как белые списки.
Мобильные операторы связи продолжают улучшать (сокращать) диапозоны белых списков, чтобы не пропускать лишние IP.
Провайдеры IaaS (yandex, vk cloud, etc) продолжают выкидывать на мороз пользователей, пытающихся использовать их сервисы для обхода блокировок.
РКН [*настоятельно попросил*](https://kod.ru/vpn-fail-bely-spisok) облачных провайдеров не выдавать в аренду IP из белых списков простным смертным.

Сейчас уже можно догадаться, какие цели ставит перед собой РКН. Внедрение БС под удобным предлогом позволило постепенно внедрить и обкатать сложную систему ТСПУ, заодно и плавно подготовить всех к новым реалиям интернета. Никогда такого не было и вот опять. 

РКН постепенно перешел с blacklisting принципов фильтрации пакетов к whitelisting. Если в blacklisting ТСПУ пытались анализировать пакеты на различные сигнатуры и признаки VPN-протоколов, whitelisting работает гораздо проще: у нас есть допустимые CIDR критической инфраструктуры, есть допустимые SNI для этих CIDR, мы также знаем, как у нас выглядит разрешенный трафик (порты, хендшейки, tls, другие фингерпринты) - все пакеты, что не подходят под этот белый список, отбрасываются либо полностью либо частично.
